...
const MAX_CHAIN = 3;
const STEP_MS = 420;        // duração de cada "passo" do boneco andando casa por casa
const EVENT_READ_MS = 4200; // tempo que a mensagem de pegadinha fica na tela
const TURN_ANNOUNCE_MS = 2600; // tempo que o aviso "vez de fulano" fica no centro da tela
...
let animating = false;
let animatingUids = new Set(); // jogadores cujo boneco está andando casa-por-casa agora
let lastMoveAnimTs = 0;        // evita repetir a mesma animação de movimento
let lastTurnUid = null;        // detecta troca de jogador da vez (pro aviso central)
let lastLogShown = null;       // evita repetir o mesmo toast de log
...
async function runOrderRoulette(room) {
  ...
  lastTurnUid = null; lastLogShown = null; lastMoveAnimTs = 0; animatingUids.clear();
  const players = room.players || {};
...
/* ============================================================
   TABULEIRO — construção do DOM: fita sinuosa com BOARD_SIZE casas,
   COLS colunas por fileira, fileiras alternando de direção.
   ============================================================ */
const COLS = 5; // largura da fita — 5 casas por fileira fica confortável no celular
...
function buildBoardDOM() {
  ...
  for (let row = 0; row < rows; row++) {
    const rowStartNumber = row * COLS + 1;
    const leftToRight = row % 2 === 0;
    // "lane" = a fileira em si (a fita colorida); "curve" = a curva que
    // conecta o fim de uma fileira ao começo da próxima (efeito sinuoso)
    const laneEl = document.createElement('div');
    laneEl.className = `board-lane ${leftToRight ? 'lane-ltr' : 'lane-rtl'}`;
    for (let c = 0; c < COLS; c++) {
      const num = leftToRight ? rowStartNumber + c : rowStartNumber + (COLS - 1 - c);
      if (num > BOARD_SIZE) continue;
      ...
      else cellEl.classList.add(`stripe-${row % 2}`);
      cellEl.innerHTML = `
        <span class="cell-num">${cellData.isStart ? 'Início' : cellData.isFinish ? 'Chegada' : num}</span>
    ...
    // curva de conexão entre esta fileira e a próxima (efeito de fita sinuosa)
    if (row < rows - 1) {
      const curve = document.createElement('div');
      curve.className = `lane-curve ${leftToRight ? 'curve-right' : 'curve-left'}`;
...
/* ============================================================
   ANIMAÇÃO DE MOVIMENTO (cliente) — "peão fantasma" que pisa em
   cada casa entre a posição antiga e a nova, tocando som a cada passo.
   ============================================================ */
function placeGhostPawn(uid, cellNum) {
  removeGhostPawn(uid);
  const holder = $(`#pawns-${cellNum}`);
  const p = roomState?.players?.[uid];
  if (!holder || !p) return;
  const el = document.createElement('div');
  el.id = `ghost-${uid}`;
  el.className = 'pawn stepping';
  el.style.background = colorHex(p.color);
  el.textContent = avatarEmoji(p.avatar);
  holder.appendChild(el);
}
function removeGhostPawn(uid) {
  const el = document.getElementById(`ghost-${uid}`);
  el?.remove();
}

async function animateWalkLocally(uid, from, to) {
  if (animatingUids.has(uid) || from === to) return;
  animatingUids.add(uid);
  const dir = to >= from ? 1 : -1;
  let cur = from;
  placeGhostPawn(uid, cur);
  while (cur !== to) {
    cur += dir;
    SoundSystem.play('step');
    placeGhostPawn(uid, cur);
    const cellEl = $(`#cell-${cur}`);
    cellEl?.classList.add('cell-active-step');
    cellEl?.scrollIntoView({ behavior: 'smooth', block: 'center' });
    await sleep(STEP_MS);
    cellEl?.classList.remove('cell-active-step');
  }
  removeGhostPawn(uid);
  animatingUids.delete(uid);
}

async function animateTeleportLocally(uid, from, to) {
  const cellFrom = $(`#cell-${from}`);
  const cellTo = $(`#cell-${to}`);
  SoundSystem.play('portal');
  cellFrom?.classList.add('cell-portal-out');
  await sleep(350);
  cellFrom?.classList.remove('cell-portal-out');
  cellTo?.classList.add('cell-portal-in');
  cellTo?.scrollIntoView({ behavior: 'smooth', block: 'center' });
  await sleep(400);
  cellTo?.classList.remove('cell-portal-in');
}

/* ---------- Toast genérico para qualquer mensagem nova em room.log ---------- */
function maybeShowLog(room) {
  if (room.log && room.log !== lastLogShown) {
    lastLogShown = room.log;
    toast(room.log, EVENT_READ_MS);
  }
}
...
function renderBoard(room) {
  ...
  // mostra qualquer mensagem nova (eventos, perda de rodada etc.) com tempo de leitura maior
  maybeShowLog(room);

  // dispara a animação local de movimento (andar casa por casa, ou teleporte)
  // quando o anfitrião publica um novo `moveAnim`
  if (room.moveAnim && room.moveAnim.ts !== lastMoveAnimTs) {
    lastMoveAnimTs = room.moveAnim.ts;
    if (room.moveAnim.mode === 'walk') animateWalkLocally(room.moveAnim.uid, room.moveAnim.from, room.moveAnim.to);
    else animateTeleportLocally(room.moveAnim.uid, room.moveAnim.from, room.moveAnim.to);
  }
  ...
  order.forEach(uid => {
    ...
    if (animatingUids.has(uid)) return; // este boneco está andando — o "peão fantasma" cuida dele
...
// Escolhe texto branco ou escuro dependendo da cor de fundo, pra sempre ficar legível
function readableTextColor(hex) {
  const c = hex.replace('#', '');
  const r = parseInt(c.substr(0, 2), 16), g = parseInt(c.substr(2, 2), 16), b = parseInt(c.substr(4, 2), 16);
  const lum = (0.299 * r + 0.587 * g + 0.114 * b) / 255;
  return lum > 0.6 ? '#1B2E22' : '#FFFFFF';
}

function announceTurn(player, isMe) {
  const el = $('#turn-announce');
  const bg = colorHex(player.color);
  el.style.background = `linear-gradient(160deg, ${bg}, ${bg}dd)`;
  el.style.color = readableTextColor(bg);
  el.innerHTML = isMe
    ? `🎯 É a sua vez!`
    : `⏳ Agora é a vez de:<br><strong>${avatarEmoji(player.avatar)} ${escapeHtml(player.name)}</strong>`;
  el.classList.add('show');
  SoundSystem.play('turnStart');
  clearTimeout(announceTurn._t);
  announceTurn._t = setTimeout(() => el.classList.remove('show'), TURN_ANNOUNCE_MS);
}
...
function renderTurnUI(room) {
  ...
  // aviso grande no centro da tela sempre que a vez muda de jogador
  if (currentUid !== lastTurnUid) {
    lastTurnUid = currentUid;
    announceTurn(currentPlayer, currentUid === myUid);
  }
  ...
  if (room.phase === 'rolling') {
    ...
    if (renderTurnUI._lastPhase !== 'moving' && room.diceResult) {
      ...
      const autoTxt = room.diceAuto ? ' (jogada automática — tempo esgotou!)' : '';
      toast(`🎲 ${currentPlayer.name} tirou ${room.diceResult}${room.diceSecond ? ` (dados: ${room.diceResult >= room.diceSecond ? room.diceResult : room.diceSecond}/${room.diceResult < room.diceSecond ? room.diceResult : room.diceSecond})` : ''}!${autoTxt}`, 3800);
    }
...
async function hostRollDice(auto) {
  ...
  // Obs: não gravamos o resultado do dado no campo "log" — ele já é anunciado
  // por um toast próprio (mais detalhado, com dado duplo e jogada automática) em renderTurnUI.
  await roomRef.update({ diceResult: value, diceSecond: secondRoll, diceAuto: !!auto, phase: 'moving' });
...
async function hostMovePlayer(uid, steps, chainDepth) {
  ...
  const target = Math.min(BOARD_SIZE, Math.max(1, pos + steps));
  ...
  // 1) avisa a todo mundo que o boneco vai andar de `pos` até `target` casa por casa
  //    (o cliente escuta este campo e faz a animação visual + som de passo)
  await roomRef.child('moveAnim').set({ uid, from: pos, to: target, mode: 'walk', ts: Date.now() });
  await animateSteps(pos, target, 'walk');

  // 2) só agora grava a posição final e limpa a animação — assim o boneco
  //    "chega" visualmente só depois de ter passado por todas as casas
  await db.ref(`${playersPath}/${uid}/position`).set(target);
  await roomRef.child('moveAnim').remove();
...
// Tempo que o anfitrião espera antes de seguir em frente — precisa bater
// aproximadamente com o tempo que a animação local (STEP_MS por casa) leva
// nos clientes, senão o próximo evento aparece antes do boneco "chegar".
function animateSteps(from, to, mode) {
  if (mode === 'teleport') return sleep(750); // portal/teleporte é instantâneo, não anda casa por casa
  const dist = Math.abs(to - from);
  return sleep(Math.max(500, dist * STEP_MS + 250));
}
...
async function hostResolveEvent(uid, eventId, chainDepth) {
  ...
  switch (ev.type) {
    ...
    case 'goto':
      return applyGotoAndMaybeChain(uid, resolveTarget(ev.target), chainDepth);
    case 'swap_leader': {
...
async function applyGotoAndMaybeChain(uid, target, chainDepth) {
  ...
  const from = roomState.players[uid].position;

  await roomRef.child('moveAnim').set({ uid, from, to: target, mode: 'teleport', ts: Date.now() });
  await animateSteps(from, target, 'teleport');
  await db.ref(`rooms/${roomCode}/players/${uid}/position`).set(target);
  await roomRef.child('moveAnim').remove();

  if (target === BOARD_SIZE) { await hostHandleFinish(uid); return; }
  const cellData = boardCells[target];
...
function findNextSpecial(from) {
  for (let i = from + 1; i <= BOARD_SIZE - 1; i++) if (boardCells[i].special) return i;
  return null;
}
...
function clampPos(p) { return Math.min(BOARD_SIZE, Math.max(1, p)); }

// Resolve o "target" de um evento 'goto'. Aceita um número fixo de casa,
// ou uma string "boardFrac:0.7" que aponta pra uma fração do tabuleiro —
// assim, se BOARD_SIZE mudar de novo, os portais continuam fazendo sentido.
function resolveTarget(target) {
  if (typeof target === 'string' && target.startsWith('boardFrac:')) {
    const frac = parseFloat(target.split(':')[1]);
    return clampPos(Math.round(BOARD_SIZE * frac));
  }
  return target;
}
