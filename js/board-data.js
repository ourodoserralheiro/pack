...
const EVENTS = [
  ...
  { id: 'pula_casa_fixa_alta', icon: '🌟', cat: 'surprise', label: 'Pulou direto para uma casa mais à frente!', desc: 'Um cogumelo gigante te lançou pra longe.', type: 'goto', target: 'boardFrac:0.7' },
  { id: 'pula_casa_fixa_media', icon: '🌟', cat: 'surprise', label: 'Pulou direto para o meio do tabuleiro!', desc: 'Um cogumelo gigante te lançou pra longe.', type: 'goto', target: 'boardFrac:0.4' },
];
...
// Tamanho do tabuleiro. Tudo no jogo (movimentação, geração de casas,
// grade visual) usa esta constante — para mudar o tamanho do trilho,
// basta alterar este número (múltiplo de 5 fica com o grid mais bonito).
const BOARD_SIZE = 50;

/**
 * Gera o layout das casas de forma determinística a partir de uma seed.
 * Regras seguidas do prompt (adaptadas proporcionalmente para BOARD_SIZE):
 *  - ~30% a 40% das casas são especiais
 *  - evitar casas especiais consecutivas
 *  - o último 1/5 do tabuleiro é mais tenso (maior densidade de eventos)
 *  - casa 1 = início, casa BOARD_SIZE = chegada (nunca especiais)
 */
function generateBoard(seed) {
  const rand = mulberry32(seed);
  const minSpecial = Math.round(BOARD_SIZE * 0.30);
  const maxSpecial = Math.round(BOARD_SIZE * 0.40);
  const totalSpecial = minSpecial + Math.floor(rand() * (maxSpecial - minSpecial + 1));
  const cells = new Array(BOARD_SIZE + 1).fill(null); // index 1..BOARD_SIZE usado

  // Divide o tabuleiro em duas zonas: início/meio (densidade normal) e
  // o último 1/5 das casas (zona tensa, mais eventos por casa)
  const tenseStart = Math.max(2, BOARD_SIZE - Math.round(BOARD_SIZE * 0.2) + 1);
  const normalRange = [];
  for (let i = 2; i <= tenseStart - 1; i++) normalRange.push(i);
  const tenseRange = [];
  for (let i = tenseStart; i <= BOARD_SIZE - 1; i++) tenseRange.push(i);
  ...
  cells[BOARD_SIZE].special = false;
  cells[BOARD_SIZE].isFinish = true;

  return cells; // array 1..BOARD_SIZE (index 0 não usado)
}
